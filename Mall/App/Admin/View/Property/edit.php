...
<html>
	...
	<body>
		...
		<input type="hidden" name="pid" id="" value="{{Q('get.id')}}" />
		<div class="alert alert-success">编辑属性</div>
		<div class="form-group">
			<label for="exampleInputEmail1">属性名称</label>
			<input name="pname" rows="5" cols=""  class="form-control" placeholder="" value="{{$oldData['pname']}}">
		</div>
		<div class="form-group">
			...
			<input type="radio" {{if value="$oldData['ptype']"}}checked="true"{{endif}} name="ptype" value="1" id="" />
		</div>
		<div class="form-group">
			<label for="exampleInputEmail1">属性值</label>
			<textarea name="value" rows="5" cols=""  class="form-control" placeholder="请输入类型按照|分开">{{$oldData['value']}}</textarea>
		</div>
