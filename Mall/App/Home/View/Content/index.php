...
<body>
	...
	<div id="head">
		<div class="top">
			<div class="zc-login-info">
				...
				<a href="{{U('Content/out')}}" class="login_act">退出</a>
				<?php else: ?>
				<span class="zc-login">
					Hi~欢迎来到ZOL商城，请
			...
			<ul class="zc-quick-menu">
				...
				<li>
					<?php if(isset($_SESSION['cart'])): ?>
						<a href="{{U('Cart/index')}}">
							购物车<em id="em">{{$_SESSION['cart']['total_rows']}}</em>件
						</a>
						<?php else: ?>
						<a href="{{U('Cart/index')}}">
							购物车0件
	...
	<div class="nav-box">
		...
		<ul class="nav clearfix">
			{{foreach from="$cateData" value="$vo"}}
			<li class="current">
				<a href="{{U('List/index',array('cid'=>$vo['cid']))}}">{{$vo['cname']}}</a>
			</li>
			{{endforeach}}
		</ul>
		<div class="nav-tips">
	...
	<div class="zs-wrapper">
		...
		<div class="zs-bread-crumbs">
			{{foreach from="$newData" value="$vb"}}
			<a href="./Public/Home/Index/index.html">首页</a>
			>
			<a href="./Public/Home/List/index.html">{{$vb['cname']}}</a>
			>
			<a href="">{{$vb['bname']}}</a>
			{{endforeach}}
		</div>
		<!--公共导航-->
		<form action="{{U('Content/buynow')}}" class="forml"  method="post">
		...
		<div class="zs-inner clearfix">
			<div class="zs-deal-detail">
				<div class="zs-focus">
					<div class="zs-big-pic left">
						...
						<a class="MagicZoom" href="">
							<img class="mid-img" src="{{pathinfo($v['gallerys'][0]['mid'],PATHINFO_DIRNAME).'/'.pathinfo($v['gallerys'][0]['mid'],PATHINFO_BASENAME)}}" alt="" style="width:400px;height:400px;"  list="{{$v['listpic'][0]}}"/>
							<input type="hidden" name="listpic" value="{{pathinfo($v['gallerys'][0]['mid'],PATHINFO_DIRNAME).'/'.pathinfo($v['gallerys'][0]['mid'],PATHINFO_BASENAME)}}" />
						</a>
					</div>
					 <div class="right">
    					<img class="big-img img" src="{{pathinfo($v['gallerys'][0]['big'],PATHINFO_DIRNAME).'/'.pathinfo($v['gallerys'][0]['big'],PATHINFO_BASENAME)}}" alt="" class="large"/>
    				 </div>
					<div class="zs-focus-thumb">
						<div class="zs-focus-list">
							<ul>
							{{foreach from="$v['gallerys']" value="$v2"}}
								<li class="show">
									<?php 
										...
										?>
									<a class="lit" imgs="{{$mid}},{{$big}}" href="javascript:;"><img src="{{pathinfo($v2['sml'],PATHINFO_DIRNAME).'/'.pathinfo($v2['sml'],PATHINFO_BASENAME)}}" alt=""  /></a>
									<i style="width:-4px;"></i>
								</li>
								{{endforeach}}
							</ul>
				...
				<div class="zs-deal-wrap">
					<!--商品标题-->
					<h3 class="zs-commodity-title" name="{{$v['gname']}}" id="{{$v['gid']}}" >
					...
					<div class="zs-price-panel">
						<dl class="zs-sale-price">
							...
							<dd>
								<span class="zs-price">
									￥
									<em price="{{$v['mprice']}}" class="mprice" >{{$v['mprice']}}</em>
									<input type="hidden" name="mprice" value="{{$v['mprice']}}" />
								</span>
							</dd>
						</dl>
					</div>
                    ...
                    <div class="name">无忧退换</div>
						...
						<div class="zs-skin-inner">
							<!--颜色-->
							{{foreach from="$pro" value="$v4"}}
							<dl class="zs-colour-type zs-suit">
								<dt name="pname">{{$v4[0]['pname']}}：</dt>
								<dd>
									<ul class="zs-options clearfix">
										{{foreach from="$v4" key="$k" value="$v3"}}
										<li class='cur{{if value="$k eq 0"}}active{{endif}}' gpid="{{$v3['gpid']}}">
											<input type="hidden" name="gpid" value="{{$v3['gpid']}}" class="korea" />
											<input type="hidden" name="options" value="" />
											<span class="attr can-buy"  name="gpvalue">
												<input type="hidden" name="{{$v4[0]['pname']}}" value="{{$v3['gpvalue']}}" />
												{{$v3['gpvalue']}}
												<i></i>
											</span>
										</li>
							...
							{{endforeach}}
							{{foreach from="$list" value="$v5"}}
								<div class="zs-suit-infor glname" groupid="{{$v5['grouppid']}}" name="glname">{{$v5['glname']}}</div>
								<input type="hidden" name="glname" value="{{$v5['glname']}}" />
								{{endforeach}}
							<!--套餐-->
	...
	<!--加入购物车-->
		...
		<div class="zs-box">
			...
			<ul class="zs-product-list clearfix">
				{{foreach from="$newGoods" value="$v7"}}
				<li>
					<a href="{{U('Content/index',array('gid'=>$v7['gid']))}}">
						<span class="zs-pic">
							<img src="{{$v7['listpic']}}" alt="" style="width: 150px;height: 150px;"/>
						</span>
						<span class="zs-title">{{$v7['gname']}}</span>
					</a>
					<div class="zs-price">
						￥
						<span>{{$v7['mprice']}}</span>
		...
		<div class="zs-side">
			...
			<div class="zs-modbox">
				...
				<ul class="zs-sales-news">
					<li>
						<a class="z_lookmore_class" href="{{U('Content/index',array('gid'=>$newGoods[0]['gid']))}}">
							<img src="{{$newGoods[0]['listpic']}}" alt="" style="width:168px;"/>
						</a>
						<div class="zs-price">
							<span class="zs-sale-price">
								￥
								<em>{{$newGoods[0]['mprice']}}</em>
							</span>
						</div>
						<span class="title">
							<a href="{{U('Content/index',array('gid'=>$newGoods[0]['gid']))}}">{{$newGoods[0]['gname']}}</a>
						</span>
					</li>
				</ul>
			</div>
			...
			<div class="zs-modbox">
				...
				<ul class="ranking-list" style="display: block;">
					{{foreach from="$newGoods" value="$var"}}
					<li>
						<a class="pic" href="{{U('Content/index',array('gid'=>$var['gid']))}}">
							<img src="{{$var['listpic']}}" alt="" style="width: 60px;height: 60px;"/>
						</a>
						<div class="title">
							<a href="{{U('Content/index',array('gid'=>$var['gid']))}}">{{$var['gname']}}</a>
						</div>
						<span class="price">{{$var['mprice']}}</span>
