		...
		<div class="wrapper clearfix contain">
			...
			<div class="right-content order-section">
				<div class="self-detail">
					<div class="order-block">
						<ul class="order-tab clearfix">
							<li>
								<a class="selected" href="{{U('MyOrder/index')}}">我的订单</a>
							</li>
							<li>
								<a href="">团购订单</a>
							</li>
					...
					<div class="addition clearfix">
                        <p class="message">
                        	<span class="gray orid" orid="{{$v['orid']}}">订单号：</span>
                        	<span>{{$v['orderNumber']}}</span>
                        	<input type="hidden" class="hao" value="{{$v['orderNumber']}}" />
                        </p>
                        <p class="message time">
                        	<span class="gray">下单时间：</span>
                        	{{date('Y-m-d H:i:s',$v['riseTime'])}}
                        </p>
                        ...
                        <span class="cancel-order" orid="{{$v['orid']}}" orderNumber="{{$v['orderNumber']}}" consignee="{{$v['consignee']}}" recieveAddress="{{$v['recieveAddress']}}" riseTime="{{$v['riseTime']}}" orderStatus="{{$v['orderStatus']}}" clid="{{$_SESSION['clid']}}" remark="{{$v['remark']}}" priceAggregate="{{$v['priceAggregate']}}">
                ...
                <table class="order-content order-table">
                        <tbody>
                        {{foreach from="$v['son']" value="$vttt"}}
                            <tr>
                                <td class="product-detail">
                                    <div class="clearfix">
                                    <a href="" class="pic" target="_blank">                                     
                                        <img alt="" src="{{$vttt['goods_listpic']}}" style="width: 60px;height: 60px;">                                       
                                    </a>
                                    <div class="des">
                                    ...
                                    <p class="price">{{$vttt['goods_price']}}</p>
                                    <p class="num">{{$vttt['amount']}}</p>
                                    </div>
                                    </td>
                                <td class="cell5 order-money">
                                    <p class="price">{{$vttt['subtotalPrice']}}</p>
                                    <p class="free">
                                        （免运费）                                    </p>
                                </td>
                                <td class="cell6" orderStatus="{{$v['orderStatus']}}">
										...
										{{elseif value="$v['orderStatus'] eq 4"}}
										<p class="red" style="color:#FF5500">订单已失效</p>
										{{endif}}
                                   
                                </td>
                                <td class="cell7" orid="{{$v['orid']}}" orderNumber="{{$v['orderNumber']}}" consignee="{{$v['consignee']}}" recieveAddress="{{$v['recieveAddress']}}" riseTime="{{$v['riseTime']}}" orderStatus="{{$v['orderStatus']}}" clid="{{$_SESSION['clid']}}" remark="{{$v['remark']}}" priceAggregate="{{$v['priceAggregate']}}">
                                    <p>
				...
				<div class="buyer-section shadow">
					...
					<div id="zp-goods-recommend">
						<ul style="list-style-type: none; overflow: hidden;" class="clearfix buyer-list recommend-content">
							...
							<li>
								<a href="{{U('Content/index',array('gid'=>$v['gid']))}}" target="_blank">
									<img src="{{$v['listpic']}}" alt="" width="180" height="180">
										<p class="price">
											<em>￥</em> 
											{{$v['mprice']}}
										</p>
